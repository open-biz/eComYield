...
import { X, ArrowUp, AlertCircle, Loader2 } from "lucide-react";
...
export function WithdrawModal({ isOpen, onClose, vaultName, deposited, earnedYield }: WithdrawModalProps) {
  ...
  const isValidAmount = withdrawAmount > 0 && withdrawAmount <= deposited;
        ...
        {withdrawSuccess ? (
          ...
          <div className="text-center py-8">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#0A2E20]/10">
              <ArrowUp size={32} className="text-[#0A2E20]" />
            </div>
            <h3 className="text-xl font-bold text-[#1C1B18]">Withdrawal Successful!</h3>
            ...
            <div className="mt-6 rounded-sm bg-[#EBE8DE] p-4 text-left">
              <div className="flex justify-between text-sm">
                <span className="text-[#1C1B18]/50">Withdrawn</span>
                <span className="font-medium text-[#1C1B18]">${withdrawAmount.toLocaleString()} USDC</span>
              </div>
              {earnedYield > 0 && (
                <div className="mt-2 flex justify-between text-sm">
                  <span className="text-[#1C1B18]/50">Yield Earned</span>
                  <span className="font-medium text-[#0A2E20]">${earnedYield.toLocaleString()} USDC</span>
                </div>
              )}
            </div>
            <button
          ...
          /* Withdraw Form */
          <>
            <h2 className="text-xl font-bold text-[#1C1B18]">Withdraw USDC</h2>
            <p className="mt-1 text-sm text-[#1C1B18]/50">{vaultName}</p>

            {/* Current Position */}
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="rounded-sm bg-[#EBE8DE] p-3">
                <span className="text-xs text-[#1C1B18]/50">Deposited</span>
                <p className="text-lg font-bold text-[#1C1B18]">${deposited.toLocaleString()}</p>
              </div>
              <div className="rounded-sm bg-[#EBE8DE] p-3">
                <span className="text-xs text-[#1C1B18]/50">Earned Yield</span>
                <p className="text-lg font-bold text-[#0A2E20]">${earnedYield.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 flex items-center justify-between">
                <span className="text-xs text-[#1C1B18]/40">
                  Available: {deposited.toLocaleString()} USDC
                </span>
                <button
            ...
            {isValidAmount && (
              <div className="mt-4 rounded-sm bg-[#EBE8DE] p-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#1C1B18]/60">You will receive</span>
                  <span className="text-sm font-bold text-[#1C1B18]">
                    ${withdrawAmount.toLocaleString()} USDC
                  </span>
                </div>
                {earnedYield > 0 && withdrawAmount === deposited && (
                  <div className="mt-2 flex items-center justify-between border-t border-[#1C1B18]/10 pt-2">
                    <span className="text-sm text-[#1C1B18]/60">Including yield</span>
                    <span className="text-sm font-bold text-[#0A2E20]">+${earnedYield.toLocaleString()} USDC</span>
                  </div>
                )}
              </div>
            )}
            ...
            <button
              ...
              disabled={!isValidAmount || isWithdrawing || !isConnected}
            ...
            >
              {isWithdrawing ? (
                ...
                <>
                  <ArrowUp size={16} />
                  Withdraw {withdrawAmount > 0 ? `${withdrawAmount.toLocaleString()} USDC` : ""}
                </>
              )}
            </button>
            ...
            <div className="mt-4 flex items-start gap-2 rounded-sm bg-[#F5F3EC] p-3">
              ...
              <p className="text-xs leading-relaxed text-[#1C1B18]/50">
                Withdrawals are processed at epoch boundaries. Your earned yield will be included in full withdrawals.
              </p>
            </div>
          </>
        )}
